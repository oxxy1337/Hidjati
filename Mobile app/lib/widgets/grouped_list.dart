...
class GroupedListView<T, E> extends StatefulWidget {
  final E Function(Surah element) groupBy;
  final Widget Function(E value) groupSeparatorBuilder;
  final Widget Function(BuildContext context, Surah element) itemBuilder;
  final Widget Function(BuildContext context, Surah element, int index)
      indexedItemBuilder;
  final GroupedListOrder order;
  final bool sort;
  final bool useStickyGroupSeparators;
  final Widget separator;
  final List<Surah> elements;
  final Key key;
...
class _GroupedLisdtViewState<T, E> extends State<GroupedListView<T, E>> {
  ...
  List<Surah> _sortElements() {
    List<Surah> elements = widget.elements;
    if (widget.sort && elements.isNotEmpty) {
      elements.sort((e1, e2) {
        ...
        if (compareResult == 0) {
          if (e1 is Comparable) {
            compareResult = (e1.juz.index).compareTo(e2.juz.index);
          } else {
    ...
    elements.sort((s1, s2){
      return int.parse(s1.index).compareTo(int.parse(s2.index));
    });
    return elements;
  }
